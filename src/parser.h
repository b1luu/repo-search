...
// Parsed representation of a single .py source file.
...
// Parse a single .py file.  Returns nullopt on I/O error.
// Extracts tokens via tokenizer and imports via regex-like line scanning.
std::optional<ParsedFile> parse_file(const std::filesystem::path& path);

// Recursively walk `root`, parse every .py file, and return results.
